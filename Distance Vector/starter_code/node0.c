...
static int recompute_mincost0(void)
{
  ...
  for (int dest = 0; dest < 4; dest++) {
    ...
    for (int via = 0; via < 4; via++) {
      if (dt0.costs[dest][via] < best)
        best = dt0.costs[dest][via];
    }

    /* update distance vector for the current dest if a better cost was found */
    if (best != mincost0[dest]) {
      mincost0[dest] = best;
      dv_changed = 1;
    }
  }
...
static void send_mincost_to_neighbors0(void)
{
  for (int n = 0; n < 4; n++) {
    ...
    pkt.destid = n;
    for (int i = 0; i < 4; i++) 
      pkt.mincost[i] = mincost0[i];

    if (TRACE) {
      printf("[t=%6.2f] node0: send its DV to node%d, DV = [", clocktime, n);
      for (int i = 0; i < 4; i++) {
...
{
  if (TRACE)
    printf("[t=%6.2f] node0: rtinit0() called\n", clocktime);

  /* initialize all costs to INF: dt[i][j] = INF */
  for (int dest = 0; dest < 4; dest++)
    for (int via = 0; via < 4; via++)
      dt0.costs[dest][via] = INF;

  /* update distance table's diagonals: dt[n][n] = cost(0, n) */
  for (int n = 0; n < 4; n++)
    dt0.costs[n][n] = connectcosts0[n];
  ...
  if (TRACE) {
    printf("[t=%6.2f] node0: initial DT:\n", clocktime);
    printdt0(&dt0);
  }

  /* send distance vector to neighbours */
  send_mincost_to_neighbors0();
}
...
void rtupdate0(struct rtpkt* rcvdpkt)
{
  ...
  if (TRACE) {
    printf("[t=%6.2f] node0: rtupdate0() called from node%d with new DV = [", clocktime, src);
    for (int i = 0; i < 4; i++) {
  ...
  if (connectcosts0[src] >= INF) {
    if (TRACE) 
      printf("[t=%6.2f] node0: packet ignored (src not neighbor)\n", clocktime);
  ...
  /* update distance table: dt[dest][src] = cost(0, src) + src_mincost[dest] */
  for (int dest = 0; dest < 4; dest++) {
    int newcost = connectcosts0[src] + rcvdpkt->mincost[dest];
  ...
  if (dt_changed)
    dv_changed = recompute_mincost0();

  if (TRACE) {
    printf("[t=%6.2f] node0: DT_updated=%s, DV_updated=%s, DT:\n", clocktime, (dt_changed ? "YES" : "NO"), (dv_changed ? "YES" : "NO"));
    printdt0(&dt0);
  }
...
{
  if (TRACE)
    printf("[t=%6.2f] node0: linkhandler0(linkid=%d, newcost=%d)\n", clocktime, linkid, newcost);
  ...
  if (TRACE) {
    printf("[t=%6.2f] node0: after link change, DV_changed=%s, DT:\n", clocktime, (dv_changed ? "YES" : "NO"));
    printdt0(&dt0);
  }
