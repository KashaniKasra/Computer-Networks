...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        if TRACE > 0:
            print('A.init(): EntityA created')

        self.seqnum_limit = seqnum_limit

        self.base = 0

        self.window_size = min(8, self.seqnum_limit - 1)
        ...
        self.timeout_interval = 20

    def _is_window_full(self):
        if self.base <= self.next_seqnum:
            in_flight = self.next_seqnum - self.base
        else:
            in_flight = self.next_seqnum + self.seqnum_limit - self.base

        return in_flight >= self.window_size
    
    def _is_ack_in_window(self, acknum):
        if self.base == self.next_seqnum:
            return False

        if self.base <= self.next_seqnum:
            return self.base <= acknum < self.next_seqnum
        else:
            return acknum >= self.base or acknum < self.next_seqnum
    ...
    def output(self, message):
        if self._is_window_full():
            if len(self.buffer) < self.buffer_max:
                if TRACE > 0:
                    print('A.output(): window full, buffering new message')

                self.buffer.append(message)
            else:
                if TRACE > 0:
                    print('A.output(): window AND buffer full, dropping new message')

            return
        
        payload = message.data
        
        packet = Pkt(seqnum=self.next_seqnum,
                  acknum=0,
                  checksum=0,
                  payload=payload)
        ...
        if TRACE > 0:
            print(f'A.output(): sending packet {packet.seqnum}')

        to_layer3(self, packet)

        self.unacked_packets[packet.seqnum] = packet

        if self.base == self.next_seqnum:
            start_timer(self, self.timeout_interval)
    ...
    def input(self, packet):
        if is_corrupted(packet):
            if TRACE > 0:
                print('A.input(): received corrupted ACK, ignoring')

            return
        
        if not self._is_ack_in_window(packet.acknum):
            if TRACE > 0:
                print(f'A.input(): received ACK {packet.acknum} not in window, ignoring')
            return
        
        if TRACE > 0:
            print(f'A.input(): received ACK {packet.acknum} in window')

        new_base = (packet.acknum + 1) % self.seqnum_limit
        ...
        while self.buffer and not self._is_window_full():
    ...
    def timer_interrupt(self):
        if TRACE > 0:
            print(f'A.timer_interrupt(): timeout, retransmitting all unacked packets')
        ...
        while seq != self.next_seqnum:
            ...
            if packet is not None:
                if TRACE > 0:
                    print(f'A.timer_interrupt(): retransmitting unacked packet {seq}')
...
class EntityB:
    ...
    def __init__(self, seqnum_limit):
        if TRACE > 0:
            print('B.init(): EntityB created')

        self.seqnum_limit = seqnum_limit

        self.expected_seqnum = 0

        self.last_ack = Pkt(seqnum=0,
                            acknum=(self.expected_seqnum - 1) % self.seqnum_limit,
                            checksum=0,
                            payload=(b'\x00' * Msg.MSG_SIZE))
        self.last_ack.checksum = compute_checksum(self.last_ack)
    ...
    def input(self, packet):
        if is_corrupted(packet):
            if TRACE > 0:
                print('B.input(): received corrupted packet, resending last ACK')
        ...
        if packet.seqnum != self.expected_seqnum:
            if TRACE > 0:
                print(f'B.input(): received unexpected seqnum {packet.seqnum}, expected {self.expected_seqnum}, resending last ACK')
        ...
        if TRACE > 0:
            print(f'B.input(): received correct packet {packet.seqnum}')
        
        message = Msg(packet.payload)

        if TRACE > 0:
            print(f'B.input(): sending message {message.data} to application')
        ...
        if TRACE > 0:
            print(f'B.input(): sending ACK {ack.acknum}')
