...
class EntityA:
    ...
    def output(self, message):
        if self.waiting_for_ack:
            if TRACE > 0:
                print('A.output(): still waiting for ACK, dropping new message')
    ...
    def input(self, packet):
        ...
        if packet.acknum != self.next_seqnum:
            if TRACE > 0:
                print(f'A.input(): received unexpected ACK {packet.acknum}, expected {self.next_seqnum}, ignoring')

            return
        
        if TRACE > 0:
            print(f'A.input(): received correct ACK {packet.acknum}')
    ...
    def timer_interrupt(self):
        if TRACE > 0:
            print(f'A.timer_interrupt(): timeout, retransmitting last packet {self.last_packet.seqnum}')
